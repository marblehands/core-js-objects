...
function mergeObjects(objects) {
  ...
  objects
    ...
    .map((array) => array.forEach((item) => newarr.push(item)));
  const newarr2 = [];
  newarr.forEach((item) => {
    const copies = newarr.filter((arr) => arr[0] === item[0]);
    let sum = 0;
    copies.forEach((elem) => {
      sum += elem[1];
    });
    newarr2.push([item[0], sum]);
  });
  const obj = {};
  newarr2.forEach(([key, value]) => {
    obj[key] = value;
  });
  return obj;
}
...
function removeProperties(obj, keys) {
  const linkToObject = obj;
  keys.forEach((key) => delete linkToObject[key]);
  return obj;
}
...
function sellTickets(queue) {
  ...
  for (let i = 0; i < queue.length; i += 1) {
    const diff = queue[i] - store.ticketCost;
    if (diff === 0) {
      store.change += queue[i];
    } else {
      store.change -= diff;
    }
    if (store.change < 0) {
...
function group(array, keySelector, valueSelector) {
  ...
  const allKeys = array.map((item) => keySelector(item));
  const uniqueKeys = new Set(allKeys);
  Array.from(uniqueKeys).forEach((key) => {
    const newArray = array.filter((item) => keySelector(item) === key);
    const uniqueValues = newArray.map((item) => valueSelector(item));
    newMap.set(key, uniqueValues);
  });
  return newMap;
}
